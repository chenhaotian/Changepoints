...
This repository provides a modern explanation and extension to the Bayesian change point model proposed by Adam and MacKay[^1].  The algorithms are implemented with R and C++, model definitions and proves are provided in this README file.
...
The graphical representation of the generating process of $\{x_t\},t=1,...,T$ shows as follow:

![](./notes_pictures/graphicModel.png)

Where  $r_t$ is the time length since the last change point at $t$, called "the run length at time $t$". $\theta_t$ is the parameter of the observation model at time $t$, $\theta_t \in \Theta_t$. $\theta_d$ is the parameter of the duration model, $\theta_d \in \Theta_d$. $\eta_d$ is the hyper-parameter of $\theta_d$, $\eta_x$ is the hyper-parameter of $\theta_t$.
...
Where $\mathcal{H}_d$ is the prior for the duration distribution, $\mathcal{H}_x$ is the prior for the observation distributions. $\mathcal{E}(x_t|\theta_t)$ is the observation distribution at time $t$,  $\mathcal{S}_{\theta_d^{(1)}}(i) = P(d_t \ge i | \theta_d=\theta_d^{(1)})$ is the survival function of the duration distribution when $\theta_d = \theta_d^{(1)}$.

The model can fit to various scenarios by flexibly adjusting the forms of $\mathcal{H}_x$, $\mathcal{H}_d$, $\mathcal{E}$ and $\mathcal{S}$.
...
A typical scenario is we first need to learn $\theta_d$ from historical data, then apply the learned $\theta_d$ to online detection problems. See [R Examples](#r-examples).
